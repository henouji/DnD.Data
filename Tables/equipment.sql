CREATE TABLE equipment (
    ...
    equipment_armor_class_id INT NULL,
    ...
    weapon_range AS (
        CASE 
            WHEN range_normal = 5 AND (range_long IS NULL OR range_long <= 5) THEN 'Melee'
            WHEN range_normal > 5 OR range_long > 5 THEN 'Ranged'
            ELSE NULL
        END
    ) PERSISTED,
    ...
    category_range AS (
        CASE 
            WHEN weapon_category IS NOT NULL THEN 
                weapon_category + ' ' + 
                CASE 
                    WHEN range_normal = 5 AND (range_long IS NULL OR range_long <= 5) THEN 'Melee'
                    WHEN range_normal > 5 OR range_long > 5 THEN 'Ranged'
